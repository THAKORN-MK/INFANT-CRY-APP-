"""Corrected grouped five-fold training for CryInsight Stage 2.

The safe default modes (``--audit-only`` and ``--prepare-only``) never import
TensorFlow or librosa.  Full five-fold training starts only with ``--train``.
All publication artefacts are written to a new immutable ``runs/<run_id>``
directory; legacy stable model files are never replaced by this script.
"""
...
def audit_stage2(data_dir: Path) -> tuple[CohortResolution, dict[str, Any]]:
    candidates = discover_stage2_candidates(data_dir, label_order=LABEL_ORDER)
    resolution = resolve_stage2_records(candidates)
    candidate_counts = Counter(row.source_label for row in candidates)
    ...
    payload: dict[str, Any] = {
        "schema_version": "1.0",
        "stage": "stage2_main_five_class",
        "data_root": str(data_dir.resolve()),
        "candidate_original_count": len(candidates),
        "candidate_counts_by_source_label": {
            label: candidate_counts[label] for label in LABEL_ORDER
        },
        ...
        "excluded_count": len(candidates) - len(resolution.eligible),
        "exclusion_counts": dict(sorted(exclusion_counts.items())),
        "duplicate_policy": {
            "same_label_exact_sha256": "keep_lexicographically_first_canonical",
            "cross_label_exact_sha256": "exclude_entire_hash_group",
        },
        "group_rule": "exact_content_sha256",
        "grouping_limitation": (
            "No verified infant subject/session identifiers were found; grouping "
            "prevents exact-content families only, so interpretation remains "
            "clip-level corrected internal validation."
        ),
        "accuracy_status": (
...
def _create_prepared_run(
    *,
    ...
    run_config = {
        ...
        "mixup_samples": args.mixup_samples,
    ...
    write_json_atomic(run_dir / "labels_main_dbl.json", list(LABEL_ORDER))
...
def train_final_model(
    *,
    ...
    """Refit Stage 2 on all development data, then open the locked test once."""
    ...
    model_path = deployment_dir / "best_model_main_dbl.keras"
    normalizer_path = deployment_dir / "norm_stats_main_dbl.npy"
    labels_path = deployment_dir / "labels_main_dbl.json"
    ...
    render_confusion_matrix_png(
        ...
        result.confusion_matrix,
        label_order=LABEL_ORDER,
        title="Stage 2 final locked internal held-out test",
    )
    test_files = {
...
def run_training(
    *,
    ...
    print(f"Stage 2 final-refit Webapp bundle: {deployment_dir}")
    ...
    verification_targets = {
        ...
        "root_labels": run_dir / "labels_main_dbl.json",
        ...
        "deployment_model": deployment_dir / "best_model_main_dbl.keras",
        "deployment_normalizer": deployment_dir / "norm_stats_main_dbl.npy",
        "deployment_labels": deployment_dir / "labels_main_dbl.json",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "CryInsight Stage 2 corrected grouped five-fold internal validation"
        )
    )
    ...
    mode.add_argument(
        "--audit-only",
        action="store_true",
        help="Hash and audit original records; do not create artefacts or import ML dependencies.",
    )
    mode.add_argument(
        "--prepare-only",
        action="store_true",
    ...
    parser.add_argument("--mixup-samples", type=int, default=500)
...
def _validate_args(parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
    ...
    if args.mixup_samples < 0 or args.bootstrap_iterations < 0:
        parser.error("--mixup-samples and --bootstrap-iterations cannot be negative")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    preprocessing = PreprocessingConfig.stage2_main()
    ...
    if args.prepare_only:
        ...
        print(f"Prepared immutable Stage 2 run: {run_dir}")
        return 0
    ...
    print(f"Completed corrected Stage 2 run: {run_dir}")
    return 0
