...
### Stage 2 data accounting: 1,551 candidates → 1,348 eligible originals

[InfantCry-DBL Version 1](https://data.mendeley.com/datasets/x493z8nmwc/1) (DOI: `10.17632/x493z8nmwc.1`) ระบุ `metadata.csv` จำนวน 1,551 clips Local dataset มีไฟล์ใน 5 infant labels จำนวนเท่ากัน และ split ปัจจุบันเก็บ candidates ครบทั้งหมดเป็น Train 1,241 กับ Test 310 records

| Exclusion reason | Train | Test | รวม |
|---|---:|---:|---:|
| Same-label exact duplicate | 110 | 7 | 117 |
| Cross-label exact duplicate | 6 | 0 | 6 |
| Exact-content Train record reserved to Test | 80 | 0 | 80 |
| **รวม exclusions** | **196** | **7** | **203** |

การกระทบยอด:

```text
Train: 1,241 − 110 − 6 − 80 = 1,045 eligible originals
Test:    310 −   7          =   303 eligible originals
รวม:   1,551 − 117 − 6 − 80 = 1,348 eligible originals
```

รายละเอียดของ exclusions:

- Same-label exact duplicates 117 records แบ่งเป็น Train 110 และ Test 7 โดยเก็บ canonical record หนึ่งรายการของ exact SHA-256 เดียวกัน
- Cross-label exact duplicates 6 records ประกอบด้วย 3 SHA-256 groups ที่แต่ละ group ปรากฏในสอง labels จึงตัดทุก record ในกลุ่มที่ label ขัดแย้ง
- Train–Test exact overlap 80 records แบ่งเป็น `belly_pain` 9, `burping` 18, `discomfort` 13, `hungry` 13 และ `tired` 27 ทุก record มี SHA-256 ตรงกับ eligible record ใน Test จึงตัดฝั่ง Train ตามกฎ Test precedence
- Eligible originals 1,348 records มี unique SHA-256 1,348 ค่า และไม่มี SHA-256 overlap ระหว่าง Train กับ Test
- ไม่มี missing files, quality filtering หรือไฟล์เสียรวมอยู่ในความต่าง 203 records และไม่มีการลบไฟล์ต้นฉบับออกจาก disk

หลักฐานที่ใช้กระทบยอด:

- [Train dataset audit](../../Models_dbl/Main/runs/20260816T102950Z_a16c5a1d/dataset_audit.json)
- [Test dataset audit](../../Models_dbl/Main/runs/20260816T102950Z_a16c5a1d/heldout_dataset_audit.json)
- [Train record-level audit](../../Models_dbl/Main/runs/20260816T102950Z_a16c5a1d/record_audit.csv)
- [Test record-level audit](../../Models_dbl/Main/runs/20260816T102950Z_a16c5a1d/heldout_record_audit.csv)
- [Held-out reservation audit](../../Models_dbl/Main/runs/20260816T102950Z_a16c5a1d/heldout_reservation.json)
...
แหล่งข้อมูลคือ InfantCry-DBL Version 1, DOI `10.17632/x493z8nmwc.1` ซึ่งหน้า Mendeley ระบุ CC BY 4.0 ก่อนส่งตีพิมพ์ต้องอ้างอิง annotation procedure, inter-rater reliability, consent และ ethics ตามเอกสารต้นฉบับ รวมถึงระบุ dataset version และ exclusion accounting ข้างต้น กลุ่ม Stage 2 เป็น operational labels ตาม Dataset ไม่ใช่การวินิจฉัยทางคลินิก
