print("!!!!! FILE VERSION CHECK - หากเห็นข้อความนี้ ไฟล์นี้คือไฟล์ที่กำลังรันจริง !!!!!")
...
BINARY_MODEL  = 'D:/INFANT CRY/models/binary/best_model_binary.keras'
BINARY_NORM   = 'D:/INFANT CRY/models/binary/norm_stats_binary.npy'
EMOTION_MODEL  = 'D:/INFANT CRY/models/cnn_mfcc/best_model_CNN_MFCC.keras'
EMOTION_LABELS = 'D:/INFANT CRY/models/cnn_mfcc/labels_CNN_MFCC.json'
EMOTION_NORM   = 'D:/INFANT CRY/models/cnn_mfcc/norm_stats_CNN_MFCC.npy'
YAMNET_URL = 'D:/INFANT CRY/models/yamnet'
BABY_CRY_THRESHOLD = 0.15
...
class BabyCryApp:
    def __init__(self, root):
        self.root = root
        self.root.title('Baby Cry Detector — CNN + MFCC')
        self.root.configure(bg=BG)
        self.root.resizable(True, True)
        self.yamnet=None; self.yamnet_classes=None
        self.emotion_model=None; self.emotion_labels=None; self.emotion_norm=None
    ...
    def _build_content(self, p):
        ...
        tk.Label(hdr,text='Emotion Detector  •  CNN + MFCC + Attention + BiLSTM',font=('Helvetica',10),bg=BG,fg=TEXT_MUTED).pack(anchor='w')
        self._div(p,pad)

        # Pipeline badges
        bf=tk.Frame(p,bg=BG); bf.pack(fill='x',padx=pad)
        for title,sub,color in [('Stage 1','YAMNet\nตรวจเสียงเด็ก','#60A5FA'),('→','',BG),('Stage 2','CNN+MFCC\nวิเคราะห์อารมณ์','#34D399')]:
            if title=='→':
        ...
        # Stage 1 result
        tk.Label(p,text='Stage 1 — ตรวจสอบเสียงเด็ก (YAMNet)',font=('Helvetica',11),bg=BG,fg=TEXT_MUTED).pack(anchor='w',padx=pad)
        ...
        self.s1_result=tk.Label(s1r,text='รอการวิเคราะห์',font=('Helvetica',14,'bold'),bg=SURFACE,fg=TEXT_MUTED)
        self.s1_result.pack(anchor='w')
        self.s1_score=tk.Label(s1r,text='',font=('Helvetica',10),bg=SURFACE,fg=TEXT_MUTED)
        self.s1_score.pack(anchor='w')
        self._div(p,pad)

        # Stage 2 result
        tk.Label(p,text='Stage 2 — วิเคราะห์อารมณ์ (CNN + MFCC)',font=('Helvetica',11),bg=BG,fg=TEXT_MUTED).pack(anchor='w',padx=pad)
    ...
    def _load_models_async(self):
        def load():
            try:
                self.status_var.set('โหลด YAMNet (Stage 1)...')
                self.yamnet=hub.load(YAMNET_URL)
                import csv
                self.yamnet_classes={}
                with open(self.yamnet.class_map_path().numpy().decode()) as f:
                    for row in csv.DictReader(f):
                        self.yamnet_classes[int(row['index'])]=row['display_name']
                self.status_var.set('โหลด Emotion Model (Stage 2)...')
                self.emotion_model=tf.keras.models.load_model(EMOTION_MODEL, custom_objects={'AttentionLayer': AttentionLayer})
                self.emotion_labels=json.load(open(EMOTION_LABELS))
                self.emotion_norm=np.load(EMOTION_NORM)
    ...
    def _pick_file(self):
        ...
        if path:
            ...
            self.s1_icon.config(text='—',fg=TEXT_MUTED)
            self.s1_result.config(text='กำลังตรวจสอบ...',fg=TEXT_MUTED)
            self.s1_score.config(text='')
    ...
    def _analyze(self):
        try:
            audio16,_=librosa.load(self.selected_file,sr=16000,mono=True)
            audio16,_=librosa.effects.trim(audio16,top_db=20)
            audio16=librosa.util.normalize(audio16).astype(np.float32)
            audio22,_=librosa.load(self.selected_file,sr=22050,mono=True)
            audio22,_=librosa.effects.trim(audio22,top_db=20)
            audio22=librosa.util.normalize(audio22)

            # Stage 1: YAMNet
            scores,_,_=self.yamnet(audio16)
            mean_scores=np.mean(scores.numpy(),axis=0)
            baby_idx=[i for i,n in self.yamnet_classes.items() if any(k in n.lower() for k in ['baby','infant','crying','child'])]
            baby_score=float(np.max([mean_scores[i] for i in baby_idx]))
            is_baby=baby_score>=BABY_CRY_THRESHOLD
            self.root.after(0,lambda: self._show_s1(is_baby,baby_score))
            if not is_baby:
                self.root.after(0,self._show_not_baby); return

            # Stage 2: CNN+MFCC
            me,se=self.emotion_norm[0],self.emotion_norm[1]
            mfcc=librosa.feature.mfcc(y=audio22,sr=22050,n_mfcc=40,n_fft=2048,hop_length=512)
            d=librosa.feature.delta(mfcc); d2=librosa.feature.delta(mfcc,order=2)
            feat=np.vstack([mfcc,d,d2])
            if feat.shape[1]<128: feat=np.pad(feat,((0,0),(0,128-feat.shape[1])))
            else: feat=feat[:,:128]
            feat=feat[...,np.newaxis].astype(np.float32)
            feat=(feat-me)/se; feat=feat[np.newaxis,...]
            probs=self.emotion_model.predict(feat,verbose=0)[0]
            top_idx=int(np.argmax(probs)); top_lbl=self.emotion_labels[top_idx]; top_pct=float(probs[top_idx])*100
    ...
    def _show_s1(self,is_baby,score):
        ...
        self.s1_score.config(text=f'Baby cry score: {score*100:.1f}%  (threshold: {BABY_CRY_THRESHOLD*100:.0f}%)')

    def _show_not_baby(self):
        self.upload_icon.config(text='❌')
        self.emoji_lbl.config(text='❌'); self.result_lbl.config(text='ไม่ใช่เสียงเด็ก',fg=RED)
    ...
    def _show_s2(self,top_lbl,top_pct,probs):
        ...
        self.ax.clear(); self.ax.set_facecolor(BG)
        sizes=[float(probs[self.emotion_labels.index(l)]) for l in LABEL_TH]
        ...
        self.ax.legend(wedges,[f"{LABEL_EN[l]}  {probs[self.emotion_labels.index(l)]*100:.1f}%" for l in LABEL_TH],loc='upper center',bbox_to_anchor=(0.5,-0.02),fontsize=9,frameon=False,labelcolor=TEXT,ncol=1,handlelength=1.5,handleheight=1.0)
